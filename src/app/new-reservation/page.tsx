...
export default function NewReservationPage() {
    ...
    function onSubmit(data: z.infer<typeof reservationFormSchema>) {
        ...
        toast({
            ...
            description: `Se ha creado una reserva para ${data.guestName} en la habitación ${data.roomId}.`,
        });
        router.push("/dashboard");
    }
    
    const availableRooms = roomsData.filter(room => room.statusText === 'Disponible');
            ...
            <main className="max-w-2xl mx-auto">
                <Form {...form}>
                    <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
                                        ...
                                        <SelectContent>
                                            {availableRooms.map(room => (
                                                <SelectItem key={room.id} value={String(room.id)}>
                                                    {room.title} - {getRoomDescription(room.price, room.id)}
                                                </SelectItem>
                                            ))}
                                        </SelectContent>
                                    </Select>
                        ...
                        <Button type="submit" className="w-full sm:w-auto">Crear Reserva</Button>
                    </form>
                </Form>
            </main>
        </div>
